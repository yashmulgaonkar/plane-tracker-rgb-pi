...
CURRENT_TEMP_POSITION_X = 40
CURRENT_TEMP_POSITION_Y = 5
...
class DaysForecastScene(object):
    ...
    def day(self, count):
        ...
        # Only fetch forecast once per hour
        ...
        if self._cached_current_temp is not None:
            temp_str = f"{self._cached_current_temp:.1f}°F"
        ...
        # Only redraw forecast if we have cached data and need to redraw
        if self._cached_forecast and self._redraw_forecast:
            self._redraw_forecast = False
            offset = 1
            space_width = screen.WIDTH // 3
            for day in self._cached_forecast:
