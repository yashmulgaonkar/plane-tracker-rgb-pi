...
In config.py set either:
    AIRLABS_API_KEY  = "single_key"           # single key
    AIRLABS_API_KEYS = ["key1", "key2", ...]  # multiple keys
"""
...
def _load_keys():
    """Load all configured AirLabs API keys."""
    try:
        from config import AIRLABS_API_KEYS
        if isinstance(AIRLABS_API_KEYS, list) and AIRLABS_API_KEYS:
            return [k for k in AIRLABS_API_KEYS if k]
    except (ImportError, AttributeError):
        pass
    try:
        from config import AIRLABS_API_KEY
        if AIRLABS_API_KEY:
            return [AIRLABS_API_KEY]
    except (ImportError, AttributeError):
        pass
    return []
