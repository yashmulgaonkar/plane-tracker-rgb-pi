...
class OpenSkyClient:
    ...
    def _refresh_token(self):
        ...
        try:
            ...
            if resp_obj is not None:
                try:
                    body = resp_obj.text[:300]
                except Exception:
                    body = ""
            if body:
                print(f"OpenSky token refresh failed: {e} | body: {body}")
            else:
    ...
    def find_callsign(self, callsign):
        """
        Search globally for a specific callsign.
        ...
        try:
            ...
            if not states:
                return None
            # Check all returned states for a match
            for s in states:
                state = _parse_state(s)
                if state and state["callsign"].strip().upper() == callsign_clean.upper():
                    return state
            # If no exact match, return first non-None result
            for s in states:
                state = _parse_state(s)
                if state:
                    return state
            ...
            print(f"[OpenSky] Callsign search failed for {callsign}: {e}")
            return None
