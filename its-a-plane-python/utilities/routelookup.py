...
def _log_usage(source, callsign, origin, dest):
    """Increment usage counter and overwrite the usage file."""
    data = _load_usage()
    ...
    def get_flight_details(self, callsign, plane_lat, plane_lon,
        ...
        if _airlabs_ok():
            ...
            if result and result.get("origin_iata"):
                _log_usage("AirLabs", callsign, result.get("origin_iata"), result.get("dest_iata"))
        ...
        if _fa_ok():
            ...
            if result and result.get("origin_iata"):
                _log_usage("FlightAware", callsign, result.get("origin_iata"), result.get("dest_iata"))
                normalised = _normalise(result, callsign, plane_lat, plane_lon)
                if normalised:
                    return _cache_and_return(normalised)
        ...
        if _has_fr24 and _fr24_client:
            ...
            if result and result.get("origin_iata") not in ("?", "", None):
                _log_usage("FR24", callsign, result.get("origin_iata"), result.get("dest_iata"))
                normalised = _normalise(result, callsign, plane_lat, plane_lon)
                if normalised:
                    return _cache_and_return(normalised)

        # All failed — cache the miss briefly so we don't hammer APIs
        from time import time
        _route_cache[callsign] = {"data": None, "ts": time()}
